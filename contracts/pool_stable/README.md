...
As an example, let's say the global ratio between two tokens x:y is 1.01:1 (1 x = 0.99 y), but the current ratio between the tokens in an Dex pair is 1:1.01 (1 x = 1.01 y). Let's also say that someone may decide to LP in the x:y Dex pool at the current 1:1.01 ratio. As the Dex pool gets arbitraged to the global ratio, the amount of x & y tokens that the LP can withdraw changes because the total amounts of x & y tokens in the pool also change.
...
Dex has two options to protect traders against slippage during swaps:
...
Please note that Dex has the default value for the spread set to 0.5% and the max allowed spread set to 50%.
